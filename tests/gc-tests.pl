#!/usr/bin/perl -w
my $tempdir = "./regression-tests/temp";
if(!(-e $tempdir)) {
  mkdir $tempdir;
}
...
my @failed_int_file;
my @successed_file;
$dirname = "./regression-tests";
chdir $dirname;
$dirname = "./";
opendir ( DIR, $dirname ) || die "Error in opening dir $dirname\n";
print("\n============= regression tests ================\n");
while( ($filename = readdir(DIR))){
  if(-d $filename and $filename ne "." and $filename ne ".." and $filename ne "temp") {
    use Cwd;
    my $predir = getcwd;
    chdir $filename;
    my @files = <test_gc_*.js>;
    ...
    foreach $fullname (@files) {
      ...
      $res = system("../../../../mapleall/maplevm/build/interpreter32 ../temp/$mmpl_file -D >> ../temp/$log_file");
      if ($res > 0) {
