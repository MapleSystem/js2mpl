    ...
    BaseNode *JSCompiler::CompileOpBinary(JSOp opcode, BaseNode *op0, BaseNode *op1) {
        ...
        switch (opcode) {
            ...
            case JSOP_MOD:
                ...
                break;
            case JSOP_ADD:
                return CompileGeneric2(INTRN_JSOP_ADD, CheckConvertToJSValueType(op0), CheckConvertToJSValueType(op1),
                                       true);
            default:
                break;
        }
        if (mop != 0) {
            ...
            if (opcodeinfo.IsCompare(mop)) {
                ...
                return jsbuilder_->CreateExprBinary(mop, restype, CheckConvertToJSValueType(op0),
        ...
        return jsbuilder_->CreateExprIntrinsicop2(idx, retty, CheckConvertToJSValueType(op0),
                                                  CheckConvertToJSValueType(op1));
    }
...
#undef JSBUILTIN_STRING_DEF
    ...
    BaseNode *JSCompiler::CompileOpString(JSString *str) {
        ...
        if (id != -1) {
            BaseNode *expr =
                CompileGeneric1((MIRIntrinsicId) INTRN_JS_GET_BISTRING, jsbuilder_->GetConstUInt32(id), false);
    ...
    BaseNode *JSCompiler::CompileOpIterNext(BaseNode *iterator) {
        ...
        StmtNode *stmt =
            jsbuilder_->CreateStmtIntrinsicCallAssigned1((MIRIntrinsicId) INTRN_JSOP_NEXT_ITERATOR, iterator, var);
        stmt->srcpos.SetLinenum(linenum_);
        jsbuilder_->AddStmtInCurrentFunctionBody(stmt);
    ...
    bool JSCompiler::CompileOpSetElem(BaseNode *obj, BaseNode *index, BaseNode *val) {
        ...
        StmtNode *stmt = jsbuilder_->CreateStmtIntrinsicCallAssigned3(INTRN_JSOP_SETPROP, obj, index,
    ...
    bool JSCompiler::CompileOpInitPropGetter(BaseNode *obj, JSString *str, BaseNode *val) {
        ...
        StmtNode *stmt = jsbuilder_->CreateStmtIntrinsicCallAssigned3(INTRN_JSOP_INITPROP_GETTER, obj, name, val, NULL);
        stmt->srcpos.SetLinenum(linenum_);
        jsbuilder_->AddStmtInCurrentFunctionBody(stmt);
        return true;
    }
    ...
    bool JSCompiler::CompileOpInitPropSetter(BaseNode *obj, JSString *str, BaseNode *val) {
        BaseNode *name = CheckConvertToJSValueType(CompileOpString(str));
        StmtNode *stmt = jsbuilder_->CreateStmtIntrinsicCallAssigned3(INTRN_JSOP_INITPROP_SETTER, obj, name, val, NULL);
        stmt->srcpos.SetLinenum(linenum_);
        jsbuilder_->AddStmtInCurrentFunctionBody(stmt);
        return true;
    }
    ...
    bool JSCompiler::CompileOpInitElemGetter(BaseNode *obj, BaseNode *index, BaseNode *val) {
        index = CheckConvertToJSValueType(index);
        StmtNode *stmt =
            jsbuilder_->CreateStmtIntrinsicCallAssigned3(INTRN_JSOP_INITPROP_GETTER, obj, index, val, NULL);
        stmt->srcpos.SetLinenum(linenum_);
        jsbuilder_->AddStmtInCurrentFunctionBody(stmt);
        return true;
    }
    ...
    bool JSCompiler::CompileOpInitElemSetter(BaseNode *obj, BaseNode *index, BaseNode *val) {
        index = CheckConvertToJSValueType(index);
        StmtNode *stmt =
            jsbuilder_->CreateStmtIntrinsicCallAssigned3(INTRN_JSOP_INITPROP_SETTER, obj, index, val, NULL);
        stmt->srcpos.SetLinenum(linenum_);
        jsbuilder_->AddStmtInCurrentFunctionBody(stmt);
        return true;
    }
    ...
    bool JSCompiler::CompileOpSetProp(BaseNode *obj, JSString *str, BaseNode *val) {
        ...
        StmtNode *stmt = jsbuilder_->CreateStmtIntrinsicCallAssigned3(INTRN_JSOP_SETPROP_BY_NAME, obj, name,
                                                                      CheckConvertToJSValueType(val), NULL);
        stmt->srcpos.SetLinenum(linenum_);
        jsbuilder_->AddStmtInCurrentFunctionBody(stmt);
        return true;
    ...
    void JSCompiler::CompileOpCase(jsbytecode *pc, int offset, BaseNode *rval, BaseNode *lval) {
        ...
        IntrinDesc *intrindesc = &IntrinDesc::intrintable[idx];
        MIRType *retty = intrindesc->GetReturnType();
        BaseNode *expr = jsbuilder_->CreateExprIntrinsicop2(idx, retty, CheckConvertToJSValueType(rval),
                                                            CheckConvertToJSValueType(lval));
    ...
    BaseNode *JSCompiler::CheckConvertToBoolean(BaseNode *node) {
        if (IsPrimitiveInteger(node->ptyp)) {
            if (node->ptyp == PTY_u1)
                return node;
            return jsbuilder_->CreateExprTypeCvt(OP_cvt, jsbuilder_->GetUInt1(), jsbuilder_->GetPrimType(node->ptyp),
                                                 node);
        }
    ...
    BaseNode *JSCompiler::CheckConvertToUInt32(BaseNode *node) {
        if (IsPrimitiveInteger(node->ptyp))
            return node;
#ifdef DYNAMICLANG
        return jsbuilder_->CreateExprTypeCvt(OP_cvt, jsbuilder_->GetUInt32(), jsbuilder_->GetPrimType(node->ptyp),
                                             node);
#else
        BaseNode *expr = jsbuilder_->CreateExprIntrinsicop1(INTRN_JS_INT32, jsbuilder_->GetInt32(), node);
        MIRSymbol *var = CreateTempVar(jsbuilder_->GetInt32());
...
#define SIMULATESTACK(nuses, ndefs)                                                                                    \
    ...
    bool JSCompiler::CompileScriptBytecodes(JSScript *script, jsbytecode *pcstart, jsbytecode *pcend,
                                            jsbytecode **newpc) {
        jsbytecode *pc = pcstart;
        ...
        while (pc < pcend) {
            ...
            switch (op) {
                    ...
                    if (true == opstack_->flag_has_rval) {
                        ...
                        if (jsbuilder_->IsPluginFunc(func)) {
                            ...
                            BaseNode *node1 = CheckConvertToJSValueType(
                                CompileGeneric1(INTRN_JS_GET_BIOBJECT,
                ...
                case JSOP_NEWARRAY: /*90, 4, 0, 1*/ {
                    ...
                    if (length == 0) {
                        BaseNode *arr =
                            CompileGeneric1(INTRN_JS_NEW_ARR_LENGTH,
                ...
                case JSOP_INITPROP: /*93, 5, 2, 1*/ {
                    ...
                    StmtNode *stmt = jsbuilder_->CreateStmtIntrinsicCallAssigned3(
                        INTRN_JSOP_INITPROP_BY_NAME, obj, name, CheckConvertToJSValueType(val), NULL);
                    stmt->srcpos.SetLinenum(lineNo);
                    jsbuilder_->AddStmtInCurrentFunctionBody(stmt);
                    Push(obj);
                    break;
