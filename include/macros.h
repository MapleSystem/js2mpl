...
#define DEBUGPRINTfuncEnd(name) do { \
  int ind = js2mplDebugIndent; \
  Util::SetIndent(1); \
  if (js2mplDebug > 0) { \
    PrintIndentation(js2mplDebugIndent); \
...
#define DEBUGPRINTnode(node) do { \
  if (js2mplDebug > 1) { \
    PrintIndentation(js2mplDebugIndent);\
    std::cout << "  >> node: "; \
    static_cast<StmtNode *>(node)->Print(module_, 0);\
    DEBUGPRINT0;\
  } \
} while(0);

// print var = val
#define DEBUGPRINT_V_LEVEL(var, level) do { \
  if (js2mplDebug >= level) { \
    PrintIndentation(js2mplDebugIndent); \
...
// print var = val
#define DEBUGPRINT_V_LEVEL_PURE(var, level) do { \
  if (js2mplDebug >= level) { \
    PrintIndentation(js2mplDebugIndent); \
...
// print val0 val1
#define DEBUGPRINT_NN_LEVEL(var0, var1, level) do { \
  if (js2mplDebug >= level) { \
    PrintIndentation(js2mplDebugIndent); \
    std::cout << var0 << " " << var1; \
  } \
...
#define DEBUGPRINT_VV_LEVEL(var0, var1, level) do { \
  if (js2mplDebug >= level) { \
    PrintIndentation(js2mplDebugIndent); \
    std::cout << _LOCATION << " " << #var0 << " = " << var0 << ", " \
    << #var1 << " = " << var1 <<   std::endl; \
  } \
} while(0);
...
// print val0, var = val
#define DEBUGPRINT_SV_LEVEL(val0, var, level) do { \
  if (js2mplDebug >= level) { \
    PrintIndentation(js2mplDebugIndent); \
    std::cout << _LOCATION << " " << val0 << ", " \
    << #var << " = " << var <<   std::endl; \
  } \
} while(0);
